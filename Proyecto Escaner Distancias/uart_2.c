...
void UART2_IRQHandler(void) {
	...
	*b_ptr_rx=LPC_UART2->RBR; 	   			 /* lee el dato recibido y lo almacena */
	    if (*b_ptr_rx++ ==13) 				// Caracter return --> Cadena completa
	    					{
							*b_ptr_rx=0;		/* Añadimos el caracter null para tratar los datos recibidos como una cadena*/ 
							b_rx_completa = 1;/* rx completa */
							b_ptr_rx=b_buffer;	/* puntero al inicio del buffer para nueva recepción */
	    					}	
		break;
	
    
   case 0x02:								/* THRE, Transmit Holding Register empty */
		if (*b_ptr_tx!=0) LPC_UART2->THR=*b_ptr_tx++;	/* carga un nuevo dato para ser transmitido */
		else b_tx_completa=1;
		break;
...
b_ptr_tx=cadena;
b_tx_completa=0;
LPC_UART2->THR=*b_ptr_tx++;	 // IMPORTANTE: Introducir un carácter al comienzo para iniciar TX o
...
void uart2_init(int baudrate) {
    LPC_SC->PCONP |= (1<<24);
    LPC_PINCON->PINSEL0|=(1<<20)|(1<<22);// Change P0.2 and P0.3 mode to TXD0 and RXD0
